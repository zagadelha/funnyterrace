...
if ( ! class_exists( 'WXR_Importer' ) && class_exists( 'WP_Importer' ) ) :
	...
	class WXR_Importer extends WP_Importer {
		...
		protected function get_reader( $file ) {
			...
			if ( ! $status ) {
				return new WP_Error( 'wxr_importer.cannot_parse', __( 'Could not open the file for parsing', 'demo-importer-plus' ) );
			}
						...
						if ( version_compare( $this->version, self::MAX_WXR_VERSION, '>' ) ) {
							$this->logger->warning(
								sprintf(
									...
									__( 'This WXR file (version %1$s) is newer than the importer (version %2$s) and may not be supported. Please consider updating.', 'demo-importer-plus' ),
									$this->version,
		...
		protected function import_start( $file ) {
			if ( ! is_file( $file ) ) {
				return new WP_Error( 'wxr_importer.file_missing', __( 'The file does not exist, please try again.', 'demo-importer-plus' ) );
			}
		...
		public function set_user_mapping( $mapping ) {
			foreach ( $mapping as $map ) {
				if ( empty( $map['old_slug'] ) || empty( $map['old_id'] ) || empty( $map['new_id'] ) ) {
					$this->logger->warning( __( 'Invalid author mapping', 'demo-importer-plus' ) );
					...
					case 'wp:status':
						...
						if ( 'auto-draft' === $data['post_status'] ) {
							...
							return new WP_Error(
								...
								__( 'Cannot import auto-draft posts', 'demo-importer-plus' ),
								$data
							);
						}
						break;
		...
		protected function process_post( $data, $meta, $comments, $terms ) {
			...
			if ( ! $post_type_object ) {
				$this->logger->warning(
					sprintf(
						...
						__( 'Failed to import "%1$s": Invalid post type %2$s', 'demo-importer-plus' ),
						$data['post_title'],
						$data['post_type']
					)
				);
			...
			if ( $post_exists ) {
				$this->logger->info(
					sprintf(
						...
						__( '%1$s "%2$s" already exists.', 'demo-importer-plus' ),
				...
				if ( ! $this->options['fetch_attachments'] ) {
					$this->logger->notice(
						sprintf(
							...
							__( 'Skipping attachment "%s", fetching attachments disabled', 'demo-importer-plus' ),
							$data['post_title']
						)
					);
					/**
			...
			if ( is_wp_error( $post_id ) ) {
				$this->logger->error(
					sprintf(
						...
						__( 'Failed to import "%1$s" (%2$s)', 'demo-importer-plus' ),
						$data['post_title'],
						$post_type_object->labels->singular_name
					)
				);
			...
			$this->logger->info(
				sprintf(
					/* translators: %1$s is the post title, %2$s is post type. */
					__( 'Imported "%1$s" (%2$s)', 'demo-importer-plus' ),
					$data['post_title'],
					$post_type_object->labels->singular_name
				)
			);
			$this->logger->debug(
				sprintf(
					...
					__( 'Post %1$d remapped to %2$d', 'demo-importer-plus' ),
					$original_id,
					$post_id
				)
			);
		...
		protected function process_attachment( $post, $meta, $remote_url ) {
			...
			if ( ! $info ) {
				return new WP_Error( 'attachment_processing_error', __( 'Invalid file type', 'demo-importer-plus' ) );
			}
		 ...
		 * @param int   $term_id Post to associate with.
		 * @return int|WP_Error Number of term items imported on success, error otherwise.
		 */
		protected function process_term_meta( $meta, $term_id ) {
			if ( empty( $meta ) ) {
				return true;
			}
			...
			foreach ( $meta as $meta_item ) {
				 ...
				 */
				$meta_item = apply_filters( 'wxr_importer.pre_process.term_meta', $meta_item, $term_id ); // phpcs:ignore WordPress.NamingConventions.ValidHookName.UseUnderscores
				if ( empty( $meta_item ) ) {
					return false;
				}

				$key   = apply_filters( 'import_term_meta_key', $meta_item['key'], $term_id );
				$value = false;
		...
		protected function process_author( $data, $meta ) {
			...
			if ( is_wp_error( $user_id ) ) {
				$this->logger->error(
					sprintf(
						...
						__( 'Failed to import user "%s"', 'demo-importer-plus' ),
						$userdata['user_login']
					)
				);
			...
			$this->logger->info(
				sprintf(
					/* translators: %s user login name  */
					__( 'Imported user "%s"', 'demo-importer-plus' ),
					$userdata['user_login']
				)
			);
			$this->logger->debug(
				sprintf(
					...
					__( 'User %1$d remapped to %2$d', 'demo-importer-plus' ),
					$original_id,
					$user_id
				)
			);
		...
		protected function parse_term_node( $node, $type = 'term' ) {
			...
			$tag_name_flipped = array_flip( $tag_name );
			...
			foreach ( $node->childNodes as $child ) { // phpcs:ignore WordPress.NamingConventions.ValidVariableName.UsedPropertyNotSnakeCase
				...
				$key = $tag_name_flipped[ $child->tagName ]; // phpcs:ignore
				if ( 'meta' === $key ) {
					$_meta = array();
					foreach ( $child->childNodes as $child_node ) {
						if ( 'wp:meta_key' === $child_node->tagName ) {
							$_meta[0] = $child_node->textContent;
						} elseif ( 'wp:meta_value' === $child_node->tagName ) {
							$_meta[1] = $child_node->textContent;
						}
					}
					if ( isset( $_meta[0], $_meta[1] ) ) {
						$meta[] = array(
							'key'   => $_meta[0],
							'value' => $_meta[1],
						);
		...
		protected function process_term( $data, $meta ) {
			...
			$this->mapping['term_parent'][ $data['taxonomy'] ][ $term_id ] = $parent_id;
			...
			$this->logger->debug(
				sprintf(
					...
					__( 'Term %1$d remapped to %2$d', 'demo-importer-plus' ),
					$original_id,
					$term_id
				)
			);

			$this->process_term_meta( $meta, $term_id );
		...
		protected function fetch_remote_file( $url, $post ) {
			...
			if ( ! empty( $max_size ) && $filesize > $max_size ) {
				...
				$message = sprintf( __( 'Remote file is too large, limit is %s', 'demo-importer-plus' ), size_format( $max_size ) );
		...
		protected function post_process() {
			...
			if ( ! empty( $this->mapping['term_parent'] ) ) {
				$this->update_term_parent( $this->mapping['term_parent'] );
			}
		}

		protected function update_term_parent( $todo ) {
			foreach ( $todo as $taxonomy => $term_by_parent ) {
				$terms         = get_terms(
					array(
						'taxonomy'   => $taxonomy,
						'hide_empty' => false,
				...
				$terms_by_slug = array_flip( $terms );
				if ( is_array( $term_by_parent ) ) {
					foreach ( $term_by_parent as $term_id => $parent_id ) {
						if ( isset( $terms_by_slug[ $parent_id ] ) ) {
							wp_update_term( $term_id, $taxonomy, array( 'parent' => $terms_by_slug[ $parent_id ] ) );
						}
					}
				}
			}
			...
			foreach ( $todo as $post_id => $_ ) {
				$this->logger->debug(
					sprintf(
						...
						__( 'Running post-processing for post %d', 'demo-importer-plus' ),
						$post_id
					)
				);
					...
					if ( isset( $this->mapping['post'][ $parent_id ] ) ) {
						...
						$this->logger->warning(
							sprintf(
								...
								__( 'Could not find the post parent for "%1$s" (post #%2$d)', 'demo-importer-plus' ),
						...
						$this->logger->debug(
							sprintf(
								...
								__( 'Post %1$d was imported with parent %2$d, but could not be found', 'demo-importer-plus' ),
								$post_id,
								$parent_id
							)
						);
					...
					if ( isset( $this->mapping['user_slug'][ $author_slug ] ) ) {
						...
						$this->logger->warning(
							sprintf(
								...
								__( 'Could not find the author for "%1$s" (post #%2$d)', 'demo-importer-plus' ),
								get_the_title( $post_id ),
								$post_id
							)
						);
						$this->logger->debug(
							sprintf(
								...
								__( 'Post %1$d was imported with author "%2$s", but could not be found', 'demo-importer-plus' ),
								$post_id,
								$author_slug
							)
						);
				...
				if ( empty( $data ) ) {
					$this->logger->debug(
						sprintf(
							/* translators: %d is post id. */
							__( 'Post %d was marked for post-processing, but none was required.', 'demo-importer-plus' ),
							$post_id
						)
					);
					continue;
				...
				if ( is_wp_error( $result ) ) {
					$this->logger->warning(
						sprintf(
							/* translators: %1$s is the post title, %2$s is post id. */
							__( 'Could not update "%1$s" (post #%2$d) with mapped data', 'demo-importer-plus' ),
							get_the_title( $post_id ),
							$post_id
						)
					);
		...
		protected function post_process_menu_item( $post_id ) {
			...
			if ( ! empty( $menu_object ) ) {
				...
				$this->logger->warning(
					sprintf(
						/* translators: %1$s is the post title, %2$s is post id. */
						__( 'Could not find the menu object for "%1$s" (post #%2$d)', 'demo-importer-plus' ),
						get_the_title( $post_id ),
						$post_id
					)
				);
				$this->logger->debug(
					sprintf(
						...
						__( 'Post %1$d was imported with object "%2$d" of type "%3$s", but could not be found', 'demo-importer-plus' ),
						$post_id,
						$menu_object_id,
						$menu_item_type
					)
			...
			foreach ( $todo as $comment_id => $_ ) {
					...
					if ( isset( $this->mapping['comment'][ $parent_id ] ) ) {
						...
						$this->logger->warning(
							sprintf(
								...
								__( 'Could not find the comment parent for comment #%d', 'demo-importer-plus' ),
								$comment_id
							)
						);
						$this->logger->debug(
							sprintf(
								...
								__( 'Comment %1$d was imported with parent %2$d, but could not be found', 'demo-importer-plus' ),
								$comment_id,
								$parent_id
							)
						);
				...
				if ( ! empty( $author_id ) ) {
					...
					if ( isset( $this->mapping['user'][ $author_id ] ) ) {
						...
						$this->logger->warning(
							sprintf(
								/* translators: %d is comment id. */
								__( 'Could not find the author for comment #%d', 'demo-importer-plus' ),
								$comment_id
							)
						);
						$this->logger->debug(
							sprintf(
								...
								__( 'Comment %1$d was imported with author %2$d, but could not be found', 'demo-importer-plus' ),
								$comment_id,
								$author_id
							)
						);
				...
				if ( empty( $result ) ) {
					$this->logger->warning(
						sprintf(
							/* translators: %d is comment id. */
							__( 'Could not update comment #%d with mapped data', 'demo-importer-plus' ),
							$comment_id
						)
					);
					continue;
